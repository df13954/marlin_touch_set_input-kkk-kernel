...
struct input_dev inputdev = {0};
struct input_dev* p_inputdev = 0;
bool once = true;
bool twoce = true;
int me_wx = 0;
int me_wy = 0;
int me_pressure = 0;
int sign = 0;
struct synaptics_rmi4_data *one_rmi4_data = 0;
struct synaptics_rmi4_data *two_rmi4_data = 0;
struct synaptics_rmi4_fn *fhandler = 0;
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	mutex_lock(&(rmi4_data->rmi4_report_mutex));
	printk(KERN_ALERT "zeyu F12_GLOVED_FINGER_STATUS start\n");
	...
	for (finger = 0; finger < fingers_to_process; finger++) {
		...
		printk(KERN_ALERT "zeyuu x:%d y:%d\n",x,y);
		...
		printk(KERN_ALERT "zeyu x:%d y:%d\n",x,y);
		if (rmi4_data->hw_if->board_data->x_flip)
			x = rmi4_data->sensor_max_x - x;
		if (rmi4_data->hw_if->board_data->y_flip)
			y = rmi4_data->sensor_max_y - y;
		...
		case F12_GLOVED_FINGER_STATUS:
			...
			if(once){
				p_inputdev = rmi4_data->input_dev;
				memcpy(&inputdev,rmi4_data->input_dev,sizeof(struct input_dev));
				me_wx = wx;
				me_wy = wy;
				me_pressure = pressure;
				once = false;
				one_rmi4_data = rmi4_data;
			}

			input_mt_slot(p_inputdev,finger);
		...
		case F12_PALM_STATUS:
			printk(KERN_ALERT "zeyu F12_PALM_STATUS\n");
			dev_dbg(rmi4_data->pdev->dev.parent,
		...
		case F12_ERASER_STATUS:
			printk(KERN_ALERT "zeyu F12_STYLUS_STATUS\n");
...
struct synaptics_rmi4_data last_irq = {0};
;static irqreturn_t synaptics_rmi4_irq(int irq, void *data)
{
	...
	last_irq.pdev = rmi4_data->pdev;
	last_irq.input_dev = rmi4_data->input_dev;
	last_irq.stylus_dev = rmi4_data->stylus_dev;
	last_irq.hw_if = rmi4_data->hw_if;
	last_irq.rmi4_mod_info = rmi4_data->rmi4_mod_info;
	last_irq.board_prop_dir = rmi4_data->board_prop_dir;
	last_irq.pwr_reg = rmi4_data->pwr_reg;
	last_irq.bus_reg = rmi4_data->bus_reg;
	last_irq.rmi4_reset_mutex = rmi4_data->rmi4_reset_mutex;
	last_irq.rmi4_report_mutex = rmi4_data->rmi4_report_mutex;
	last_irq.rmi4_io_ctrl_mutex = rmi4_data->rmi4_io_ctrl_mutex;
	last_irq.rmi4_exp_init_mutex = rmi4_data->rmi4_exp_init_mutex;
	last_irq.rmi4_irq_enable_mutex = rmi4_data->rmi4_irq_enable_mutex;
	last_irq.rb_work = rmi4_data->rb_work;
	last_irq.rb_workqueue = rmi4_data->rb_workqueue;
	last_irq.fb_notifier = rmi4_data->fb_notifier;
	last_irq.current_page = rmi4_data->current_page;
	last_irq.button_0d_enabled = rmi4_data->button_0d_enabled;
	last_irq.num_of_tx = rmi4_data->num_of_tx;
	last_irq.num_of_rx = rmi4_data->num_of_rx;
	last_irq.num_of_fingers = rmi4_data->num_of_fingers;
	last_irq.max_touch_width = rmi4_data->max_touch_width;
	last_irq.report_enable = rmi4_data->report_enable;
	last_irq.no_sleep_setting = rmi4_data->no_sleep_setting;
	last_irq.report_data = rmi4_data->report_data;
	last_irq.report_data_32 = rmi4_data->report_data_32;
	last_irq.temp_report_data = rmi4_data->temp_report_data;
